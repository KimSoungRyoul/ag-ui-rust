...
use ag_ui_client::transport::{SseDecoder, SseFrame, decode_events};
use ag_ui_core::{Event, EventType};
use futures_util::StreamExt;

/// Decodes a whole body, however it is chunked, the way a transport would.
fn decode(chunks: &[&[u8]]) -> Result<Vec<SseFrame>, ag_ui_client::Error> {
...
fn decode_body(body: &str) -> Result<Vec<SseFrame>, ag_ui_client::Error> {
...
fn a_payload_with_embedded_newlines_and_blank_lines_round_trips() {
    ...
    let body = ag_ui_core::encode::sse::frame(payload);
...
fn an_event_is_parsed_out_of_a_frame() {
    let body = ag_ui_core::encode::sse::frame(
