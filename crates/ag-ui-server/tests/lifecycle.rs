...
async fn a_resuming_request_reaches_the_agent() {
    ...
    impl Agent for Resumes {
        ...
        async fn run(&self, ctx: &mut RunContext<()>) -> Result<RunOutcome> {
            ...
            assert_eq!(answer.status, ag_ui_core::ResumeStatus::Resolved);
            ctx.say("Deleted.")?;
            Ok(RunOutcome::Success)
        }
    }
    ...
    let mut input = input();
    input.resume = Some(vec![ag_ui_core::ResumeEntry::resolved(
        "i-1",
...
async fn the_agent_sees_the_whole_request() {
    ...
    let mut input = input();
    input.messages = vec![ag_ui_core::Message::user("m-1", "hello")];
    input.tools = vec![ag_ui_core::Tool::new(
        "search",
        "Search the web",
        serde_json::json!({"type": "object"}),
    )];
    input.context = vec![ag_ui_core::Context::new("page", "/invoices")];
...
async fn a_boxed_agent_runs_like_any_other() {
    let agent: ag_ui_server::BoxAgent<()> = Box::new(Idle);
