...
//! - the handle borrows the [`RunContext`](crate::RunContext) mutably, so a
...
/// [`RunContext`](crate::RunContext) or a handle, which is what keeps the
...
///
/// Yielded by [`RunContext::new`](crate::RunContext::new) for agents under
/// test. Transports get a [`Stream`] from
/// [`Runner::run`](crate::Runner::run) instead.
#[derive(Debug)]
