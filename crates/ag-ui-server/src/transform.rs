...
//!
//! ```
//! # use ag_ui_server::{FilterToolCalls, TransformerChain, ToolResultToState};
...
use ag_ui_core::{Event, PatchOperation, ToolCallId};
...
/// # use ag_ui_server::{FilterToolCalls, StreamTransformer};
    ...
    fn transform(&mut self, event: Event) -> Vec<Event> {
        let drop = match &event {
            ...
            Event::ReasoningEncryptedValue(payload) => {
                payload.subtype == ag_ui_core::ReasoningEncryptedValueSubtype::ToolCall
...
/// ```
/// # use ag_ui_core::Event;
/// # use ag_ui_server::{StreamTransformer, ToolResultToState};
...
mod tests {
    use super::*;
    use ag_ui_core::{ReasoningEncryptedValueSubtype, TextMessageRole};
    use serde_json::json;
