...
use ag_ui_core::encode::sse;
use ag_ui_core::{
    Event, EventStreamFormatter, RunErrorEvent, SSE_MEDIA_TYPE, SseFormatter, media_type,
};
use ag_ui_server::CancellationToken;
...
///
/// ```
/// # use ag_ui_axum::respond::negotiate;
...
/// the run — [`route_agui`](crate::RouterExt::route_agui) is this, with the
/// defaults filled in:
///
/// ```
...
impl SseResponse {
    ...
    where
        S: Stream<Item = ag_ui_server::Result<Event>> + Send + 'static,
    {
        let body = EventBody {
...
struct EventBody {
    ...
    guard: DisconnectGuard,
    events: Pin<Box<dyn Stream<Item = ag_ui_server::Result<Event>> + Send>>,
    formatter: SseFormatter,
