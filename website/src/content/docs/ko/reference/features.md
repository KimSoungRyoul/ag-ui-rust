---
title: feature flag
description: 다섯 crate의 모든 Cargo feature — 기본 상태, 무엇을 끌어오는지, 끄면 무엇을 잃는지, CI가 그 조합을 어떻게 검사하는지.
---

다섯 crate에 feature 여덟 개가 있습니다. 이 page가 완전한 목록입니다. 각 crate의 rustdoc 첫
page에도 그 crate 몫의 같은 정보가 있습니다 —
...
:::note[여기 있는 crate에 의존하는 방법]
이 다섯 crate는 crates.io에 없습니다. 그래서 dependency 줄은 version이 아니라 repository를
가리킵니다. 아래 `[dependencies.…]` block이 하는 일입니다. registry에 *실제로* 있는
`ag-ui-core`, `ag-ui-server`, `ag-ui-client`는 이전 community SDK의 것입니다. 이 project가
아닙니다. `ag-ui-axum`과 `ag-ui-a2ui`는 아예 올라가 있지 않습니다.
:::
...
| --- | --- | :---: | --- | --- |
| `ag-ui-core` | `sse` | on | — | `SseFormatter`와 `text/event-stream` framing. |
| `ag-ui-core` | `protobuf` | off | — | binary transport의 media type과 content negotiation. encoder는 없습니다. |
| `ag-ui-core` | `schemars` | off | `schemars` | 공개 type에 붙는 `schemars::JsonSchema` derive. |
| `ag-ui-core` | `utoipa` | off | `utoipa` | 공개 type에 붙는 `utoipa::ToSchema` derive. |
| `ag-ui-server` | `verify` | on | — | runtime ordering state machine. |
| `ag-ui-client` | `http` | on | `reqwest` | `HttpTransport`와 `HttpAgent`. |
...
| `ag-ui-a2ui` | `ag-ui` | on | `ag-ui-core` | AG-UI type과의 interop. `toolkit`을 함의합니다. |

`ag-ui-axum`에는 feature가 없습니다. web binding이라 하는 일 전부가 곧 존재 이유입니다.

여덟 중 dependency를 더하는 것은 넷입니다. `schemars`, `utoipa`, `http`, `ag-ui`입니다.
나머지 넷은 crate가 이미 compile하는 것에 대한 code gate입니다.

## feature 하나씩

**`ag-ui-core/sse`.** 끄면 `SseFormatter`와 content negotiation의 SSE 분기를 잃습니다.
`protobuf`까지 off면 `encode` module 전체를 잃습니다.
`any(feature = "sse", feature = "protobuf")`로 gating되기 때문입니다. protocol type만 남고,
그것을 wire용으로 framing하는 것은 남지 않습니다. `ag-ui-axum`은 `SseFormatter`를 직접 씁니다.
그래서 이 feature가 필요합니다. 기본이 on이고, 이 workspace의 어떤 것도 끄지 않습니다.

**`ag-ui-core/protobuf`.** 이 feature가 더하는 것은 하나뿐입니다. content negotiation에
...
**`ag-ui-core/schemars`, `ag-ui-core/utoipa`.** 기본이 off입니다. 둘 다 대부분의 소비자에게
필요 없는 일을 위한 dependency이기 때문입니다. protocol type을 기술해야 하는 JSON Schema나
OpenAPI 문서를 만들 때 켜십시오.

**`ag-ui-server/verify`.** 끄면 server 쪽 protocol verification을 잃습니다. verifier는 크기
...
tokio를 끌어오기 때문에, `ag-ui-client`는 `http`가 off일 때만 executor에 종속되지 않습니다.
...
**`ag-ui-a2ui/ag-ui`.** 끄면 `agui` module과 `ag-ui-core` dependency를 잃습니다. AG-UI
...
`ag-ui = ["dep:ag-ui-core", "toolkit"]`이므로 `ag-ui`만 요청해도 `toolkit`이 따라와야 합니다.
...
없음을 단언합니다. 그중 하나가 `ag-ui-client --no-default-features`입니다 —
