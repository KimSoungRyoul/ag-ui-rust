...
`ag-ui`, `ag_ui::server`, `ag_ui::client`는 tokio가 아니라 `futures`의 기본 요소를 씁니다.
...
CI 주석에 그 실험이 기록되어 있습니다. `ag_ui::server`의 `[dependencies]`에
...
`ag_ui::server`의 `[dev-dependencies]`가 `#[tokio::test]`를 위해 끌어옵니다. 이 약속이 말하는
...
이 workspace에서는 아무것도 나오지 않습니다. `ag_ui::server`의 normal graph는 자기까지 26개
