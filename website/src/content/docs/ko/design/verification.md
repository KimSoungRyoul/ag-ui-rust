...
1. **코드가 겹치는 block 두 개를 열 수 없습니다.** 타입 시스템의 보증이고,
...
3. **event 집합이 여전히 프로토콜과 맞습니다.** 저장소에 넣어 둔 upstream
...
유지됩니다. 연 것은 닫힙니다. 메시지 한복판에서 `?`가 풀려 나가도 그렇습니다.

### stable rustdoc으로는 부족한 이유

저 block의 annotation은 기대하는 오류 코드를 적습니다. `compile_fail,E0499`입니다.
**stable rustdoc은 그 코드를 파싱한 다음 무시합니다.** 예제는 compile에 실패하기만
하면 됩니다. 이유는 무엇이든 상관없습니다. 오타 하나로도 충분합니다. 그러면 test는
계속 통과하고, 보증만 조용히 검사되지 않습니다.

추측이 아닙니다. `emit/mod.rs`의 doctest에 `E0308`을 붙여 확인했습니다. 타입
...
프로토콜이 금지하는 것이 모두 borrow로 표현되지는 않습니다. `ctx.emit`은 chunk
event와 뒤섞인 병렬 tool call을 위해 문서화된 escape hatch입니다. 이것을 쓰는
agent는 아무도 열지 않은 메시지에 `TEXT_MESSAGE_CONTENT`를 emit할 수 있습니다.
...
fn main() {
    ...
    // 한 번도 열린 적 없는 메시지에 대한 content입니다.
    let error = ctx
...
| `DuplicateStart` | 이미 열려 있는 id로 메시지, reasoning block, tool call, step을 여는 것 |
...
| `OpenAtFinish` | 메시지, reasoning block, tool call, step이 열린 채로 온 `RUN_FINISHED` |

`RUN_ERROR`는 `OpenAtFinish`에서 면제됩니다. 메시지 도중에 터진 run이 그것을
닫았을 리 없습니다.

거부는 하나하나가
[`VerificationError`](/ag-ui-rust/api/ag_ui_server/error/struct.VerificationError.html)입니다.
문제가 된 event 타입과 규칙, 그리고 detail 문자열을 담습니다. 실제로 열린 메시지가
...
id는 얼마든지 겹칠 수 있습니다. 어떤 call을 서술하는 메시지 안에서 그 tool call이
...
있습니다. 사용자에게 닿는 프로토콜 bug 옆에 두면 그 값은 고민할 거리가 아닙니다.

측정해 보고 그 조회를 되찾고 싶다면 `ag-ui-server`의 `verify` feature를 끄십시오.
state machine 전체가 크기 0인 타입으로 바뀝니다. 그 타입의 `observe`는 inline된
`Ok(())`입니다.
...
`ag-ui-client`도 검증합니다. 이유는 다릅니다. event가 남의 프로세스에서
도착합니다. 규칙을 어기는 stream은 혼란스러운 UI가 아니라 분명한 오류 하나를 내야
...
- `RUN_STARTED`가 stream을 열고, 정확히 한 번만 그렇게 합니다. 그 앞에 올 수 있는
  것은 `RAW`와 `CUSTOM`뿐입니다. 둘은 정의상 프로토콜의 어휘 바깥이고, 그래서
  ordering 바깥이기도 합니다.
- `interrupt` 결과는 interrupt를 적어도 하나 실어야 합니다. 타입 시스템으로
  표현할 수 없는 유일한 규칙입니다.
...
Rust event 타입은 upstream Zod schema를 손으로 옮긴 것입니다. compiler에는 둘을
잇는 것이 없습니다. 그래서 upstream이 event를 추가해도 이 SDK는 계속 build되고,
계속 test를 통과하고, 조용히 프로토콜을 더는 말하지 못하게 됩니다. 앞선 어느
...
이것을 빨갛게 만들 수 없습니다. 종료 코드 0은 깨끗함, 1은 drift입니다. 2는
baseline이 없거나 event module이 옮겨졌다는 뜻입니다. 진짜 저장소 결함이므로 이
역시 실패해야 합니다.

추출기가 Zod schema를 확신 있게 읽지 못한 event는 `unparsed`로 기록됩니다. 타입은
...
offline 검사는 Rust 타입이 snapshot과 맞는다는 것까지만 말합니다. *snapshot*이
...
`events.json`의 diff가 **곧** 프로토콜 변경입니다. 그 pull request에서 가장 꼼꼼히
...
code에 맞춰 프로토콜을 고치는 일입니다. 이 검사가 잡으려고 존재하는 실패가 바로
그것입니다.
...
- borrow checker는 `ctx.emit`으로 나간 event를 보지 못합니다. 그래서 layer 2가
  있습니다.
- runtime verifier는 프로토콜에 34번째 event가 생겼다는 것을 알지 못합니다.
  그래서 layer 3이 있습니다.
