...
`ag_ui::client` appears with `--no-default-features` because the default `http` feature pulls
`reqwest`, which is not a wasm story this workspace tells. `ag_ui::axum` does not appear at all,
...
`ag_ui::client` is executor-agnostic **only with the `http` feature off**. `http` pulls
...
The hole is a trait —
[`Transport`](/ag-ui-rust/api/ag_ui/client/transport/trait.Transport.html) — and you fill it:
